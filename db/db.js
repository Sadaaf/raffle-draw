...
class MyDB {
  ...
  /**
   * Updates all tickets associated username with passed in information
   * @param {string} username
   * @param {{username: string, price: number}} ticketBody
   * @returns {Array<Ticket>} an array of updated tickets
   */
  updateByUsername(username, ticketBody) {
    tickets = this.tickets.filter((ticket) => ticket.username == username);
  ...
  /**
   * Deletes all tickets associated with the username
   * @param {string} username
   */
  deleteByUsername(username) {
    this.tickets.findIndex((t) => {
